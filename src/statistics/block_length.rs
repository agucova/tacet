...
///
/// ```
/// use timing_oracle::statistics::optimal_block_length;
///
/// // AR(1) process with moderate dependence
/// let data: Vec<f64> = (0..500).map(|i| {
///     if i == 0 { 0.0 } else { 0.5 * (i - 1) as f64 + 0.1 }
/// }).collect();
///
/// let opt = optimal_block_length(&data);
/// println!("Stationary: {:.1}, Circular: {:.1}", opt.stationary, opt.circular);
/// ```
...
///
/// ```
/// use timing_oracle::statistics::paired_optimal_block_length;
///
/// let baseline: Vec<f64> = (0..500).map(|i| i as f64 + 100.0).collect();
/// let sample: Vec<f64> = (0..500).map(|i| i as f64 + 100.5).collect();
///
/// let block_size = paired_optimal_block_length(&baseline, &sample);
...
mod tests {
    ...
    fn test_iid_data_small_block() {
        ...
        println!(
            "IID data: stationary = {:.2}, circular = {:.2}",
            opt.stationary, opt.circular
        );
    }
    ...
    fn test_ar1_moderate_dependence() {
        ...
        println!(
            "AR(1) φ=0.5: stationary = {:.2}, circular = {:.2}",
            opt.stationary, opt.circular
        );
    }
    ...
    fn test_ar1_strong_dependence() {
        ...
        println!(
            "AR(1) φ=0.9: stationary = {:.2}, circular = {:.2}",
            opt.stationary, opt.circular
        );
    }

    #[test]
    fn test_block_length_scaling_with_n() {
        // Block length should scale roughly as n^(1/3)
        // However, the algorithm is data-adaptive and includes estimation noise,
        // so we test a weaker property: larger samples should give comparable or larger blocks
        let phi = 0.5;

        // Run multiple times to reduce stochastic variation
        let mut ratios = Vec::new();
        for seed in 0..10 {
            let opt_200 = optimal_block_length(&generate_ar1(200, phi, seed));
            let opt_800 = optimal_block_length(&generate_ar1(800, phi, seed));

            let ratio = opt_800.circular / opt_200.circular;
            ratios.push(ratio);
        }
        ...
        println!(
            "Average ratio over 10 trials: {:.2} (expected ~1.59 asymptotically)",
            mean_ratio
        );

        // The average ratio should be in a reasonable range
        // The n^(1/3) scaling is asymptotic; for finite samples, we just check it's positive
        assert!(
            mean_ratio > 0.5 && mean_ratio < 5.0,
            "Average block length ratio {} outside reasonable bounds",
            mean_ratio
        );
    }
    ...
    #[test]
    fn test_realistic_timing_data() {
        // Simulate realistic timing measurements: base time + noise + autocorrelation
        use rand::SeedableRng;
        let mut rng = rand::rngs::StdRng::seed_from_u64(42);
        let n = 1000;
        let base_time = 100.0; // 100 ns base operation
        let noise_std = 5.0; // 5 ns noise

        // AR(1) noise component with moderate autocorrelation
        let mut x = vec![0.0; n];
        x[0] = base_time + noise_std * (rng.random::<f64>() - 0.5);

        for i in 1..n {
            let ar_component = 0.3 * (x[i - 1] - base_time);
            let innovation = noise_std * (rng.random::<f64>() - 0.5);
            x[i] = base_time + ar_component + innovation;
        }

        let opt = optimal_block_length(&x);

        println!(
            "Timing data: stationary = {:.2}, circular = {:.2}",
            opt.stationary, opt.circular
        );

        // Should give reasonable block lengths
        assert!(opt.stationary >= 1.0 && opt.stationary <= 50.0);
        assert!(opt.circular >= 1.0 && opt.circular <= 50.0);
    }
}
