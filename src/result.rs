...
impl Outcome {
    ...
    ///
    /// Returns `Some(self)` if measurement is reliable.
        ...
        let message = match &self {
            ...
            Outcome::Inconclusive { reason, leak_probability, .. } => {
                let reason_str = match reason {
                    ...
                    InconclusiveReason::SampleBudgetExceeded { .. } => "sample budget exceeded",
                };
                format!(
                    "inconclusive ({}): P(leak)={:.0}%",
                    reason_str,
            ...
            Outcome::Pass { quality, leak_probability, .. } => {
                format!(
                    "unreliable pass: P(leak)={:.0}%, quality={:?}",
                    leak_probability * 100.0, quality
                )
            }
            Outcome::Fail { quality, leak_probability, .. } => {
                format!(
                    "unreliable fail: P(leak)={:.0}%, quality={:?}",
                    leak_probability * 100.0, quality
                )
            }
        };
...
impl std::fmt::Display for EffectPattern {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
...
impl std::fmt::Display for MeasurementQuality {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
