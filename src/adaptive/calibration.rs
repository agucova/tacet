...
pub fn calibrate(
    ...
    // Set prior covariance: sigma = max(2 * MDE, theta) per spec Section 2.5
    let prior_sigma_shift = (2.0 * mde.shift_ns).max(config.theta_ns);
    let prior_sigma_tail = (2.0 * mde.tail_ns).max(config.theta_ns);
