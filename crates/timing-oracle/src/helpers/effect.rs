...
//!
//! ```ignore
//! use timing_oracle::helpers::effect::{busy_wait_ns, EffectInjector};
//!
...
fn warn_imprecise_fallback() {
    if !FALLBACK_WARNING_PRINTED.swap(true, Ordering::Relaxed) {
        eprintln!(
            "[timing-oracle::effect] WARNING: Using Instant::now() fallback for busy_wait_ns. \
...
fn calibrate_aarch64_frequency() -> u64 {
    ...
    if frequencies.is_empty() {
        eprintln!(
            "[timing-oracle::effect] WARNING: ARM64 calibration failed. Using 24MHz estimate."
        );
        return 24_000_000;
    }
    ...
    eprintln!(
        "[timing-oracle::effect] INFO: ARM64 counter frequency calibrated to {:.2} MHz",
...
fn calibrate_tsc_frequency() -> u64 {
    eprintln!(
        "[timing-oracle::effect] INFO: Calibrating TSC frequency (no sysfs/sysctl available)..."
    );
    ...
    if frequencies.is_empty() {
        eprintln!(
            "[timing-oracle::effect] WARNING: TSC calibration failed. Using 3GHz estimate."
        );
    ...
    eprintln!(
        "[timing-oracle::effect] INFO: TSC frequency calibrated to {:.2} GHz",
...
///
/// ```ignore
/// use timing_oracle::helpers::effect::EffectInjector;
///
...
///
/// ```ignore
/// use timing_oracle::helpers::effect::{BenchmarkEffect, apply_effect};
///
