...
//!
//! This module re-exports the formatting functions from `timing-oracle-core`.
//! When the `ansi` feature is enabled on `timing-oracle-core` (which it is by
//! default when using `timing-oracle`), the output includes ANSI color codes
//! automatically.
...
/// Colors are included automatically via the `ansi` feature on `timing-oracle-core`.
pub fn format_outcome(outcome: &Outcome) -> String {
    timing_oracle_core::formatting::format_outcome_plain(outcome)
}
...
pub fn format_debug_summary(outcome: &Outcome) -> String {
    timing_oracle_core::formatting::format_debug_summary_plain(outcome)
}
