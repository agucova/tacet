...
fn main() {
    ...
    for effect_ns in [0u64, 50, 100, 200, 500] {
        ...
        match &outcome {
            Outcome::Pass { effect, diagnostics, .. } => {
                println!("Effect {:4}ns | Measured: {:7.1}ns | Timer: {:.1}ns | PASS", 
                         effect_ns, effect.shift_ns, diagnostics.timer_resolution_ns);
            }
            Outcome::Fail { effect, diagnostics, .. } => {
                println!("Effect {:4}ns | Measured: {:7.1}ns | Timer: {:.1}ns | FAIL ✓", 
                         effect_ns, effect.shift_ns, diagnostics.timer_resolution_ns);
            }
            Outcome::Inconclusive { effect, diagnostics, .. } => {
                println!("Effect {:4}ns | Measured: {:7.1}ns | Timer: {:.1}ns | INCONCLUSIVE", 
                         effect_ns, effect.shift_ns, diagnostics.timer_resolution_ns);
            }
