...
Benchmark comparison infrastructure for evaluating tacet against other timing side-channel detection tools.
...
- *Non-parametric* (tacet, RTLF, SILENT, Mona): Make minimal distribution assumptions.
...
- Bootstrap-based methods (RTLF, SILENT) and tacet's covariance estimation handle dependencies.

**False Positive Rate (FPR) Control:**
- *Formal*: User specifies α, tool guarantees FPR ≤ α (tacet, RTLF, SILENT, tlsfuzzer)
...
|---------|------|-------------|
| `TimingOracleAdapter` | tacet | Full Bayesian analysis with adaptive sampling |
...
```rust
use tacet_bench::{SyntheticConfig, EffectType, generate_dataset};
...
```rust
use tacet_bench::{TimingOracleAdapter, DudectAdapter, ToolAdapter};

let dataset = generate_dataset(&config);

// tacet (uses interleaved data)
let to_adapter = TimingOracleAdapter::default();
let result = to_adapter.analyze(&dataset);
println!("tacet: detected={}, P(leak)={:.4}",
...
```rust
// tacet uses interleaved (exploits temporal correlations)
...
```rust
use tacet_bench::TlsfuzzerAdapter;
...
```rust
use tacet_bench::TimingTvlaAdapter;
...
For rigorous analysis, prefer RTLF, SILENT, or tacet.
...
```rust
use tacet_bench::AndersonDarlingAdapter;
