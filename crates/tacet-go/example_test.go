...
func ExampleWithAttacker() {
	op := tacet.FuncOperation(func(input []byte) {
		// Some operation
		var sum byte
		for _, b := range input {
			sum ^= b
	...
	_, _ = tacet.Test(
		tacet.NewZeroGenerator(1),
		op, 32,
		tacet.WithAttacker(tacet.AdjacentNetwork), // 100ns threshold
		tacet.WithTimeBudget(5*time.Second),
	)
	...
	_, _ = tacet.Test(
		tacet.NewZeroGenerator(1),
		op, 32,
		tacet.WithAttacker(tacet.RemoteNetwork), // 50us threshold
		tacet.WithTimeBudget(5*time.Second),
	)
