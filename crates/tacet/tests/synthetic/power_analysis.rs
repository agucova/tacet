...
/// Maximum fraction of trials that can be unmeasurable before skipping the test
const MAX_UNMEASURABLE_FRACTION: f64 = 0.3;
...
fn mde_calibrated_power_curve() {
    ...
    for trial in 0..MDE_ESTIMATION_TRIALS {
        ...
        if (trial + 1) % 25 == 0 {
            eprintln!(
                "  MDE estimation: {}/{} trials complete ({} unmeasurable)",
                trial + 1,
                MDE_ESTIMATION_TRIALS,
                unmeasurable_count
            );
    ...
    let unmeasurable_fraction = unmeasurable_count as f64 / MDE_ESTIMATION_TRIALS as f64;
    if unmeasurable_fraction > MAX_UNMEASURABLE_FRACTION {
        eprintln!(
            "[mde_power_curve] SKIPPED: {:.0}% of trials were unmeasurable (threshold: {:.0}%)",
            unmeasurable_fraction * 100.0,
            MAX_UNMEASURABLE_FRACTION * 100.0
        );
        eprintln!("  Try running with sudo for high-precision timing, or on a less noisy system");
        return;
    }

    if mde_estimates.is_empty() {
        eprintln!("[mde_power_curve] SKIPPED: No measurable trials");
        return;
    }
    ...
    for &multiple in &mde_multiples {
        ...
        for trial in 0..TRIALS_PER_MULTIPLE {
            ...
            if (trial + 1) % 20 == 0 && measurable_trials > 0 {
        ...
        if measurable_trials == 0 {
            eprintln!(
                "[mde_power_curve] SKIPPED {:.1}×MDE: No measurable trials",
                multiple
            );
            continue;
        }
        ...
        eprintln!(
            "[mde_power_curve] {:.1}×MDE ({:.0}ns): power={:.1}% [95% CI: {:.1}%-{:.1}%] ({} trials)",
            multiple,
            effect_ns,
            power * 100.0,
            ci_low * 100.0,
    ...
    // Assertions
    // Skip assertions if we don't have enough results
    if power_results.len() < 3 {
        eprintln!(
            "[mde_power_curve] SKIPPED: Only {} MDE multiples measured (need at least 3)",
            power_results.len()
        );
        return;
    }
