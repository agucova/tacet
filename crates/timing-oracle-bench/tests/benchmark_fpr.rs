...
//!
//! These tests measure the false positive rate of timing-oracle and comparison tools
...
fn fpr_sanity_check() {
    ...
    let to_fpr = results.fpr("timing-oracle");
    assert!(
        to_fpr < 0.5,
        "timing-oracle FPR {} is unreasonably high for sanity check",
        to_fpr
    );
}
...
fn fpr_15k_samples() {
    ...
    assert!(
        to_fpr <= 0.10,
        "timing-oracle FPR {:.1}% exceeds 10% threshold",
        to_fpr * 100.0
    );
}
...
fn fpr_multi_tool_comparison() {
    ...
    //
    // timing-oracle uses Custom { threshold_ns: 0.0 } (floored to timer resolution)
