...
//!
//! These tests measure the detection rate (power) of timing-oracle and comparison tools
...
/// Create a timing-oracle adapter configured for fair comparison with dudect/RTLF.
///
...
fn power_sanity_check() {
    ...
    let to_power = results.power("timing-oracle");
    println!("timing-oracle power: {:.1}%", to_power * 100.0);
...
fn power_30k_shift_1pct() {
    ...
    let to_power = results.power("timing-oracle");
    println!("\ntiming-oracle detects 1% shifts at {:.1}% rate", to_power * 100.0);
}

/// Power benchmark with 5% shift (clear effect).
///
/// Expected: timing-oracle power >= 90%
#[test]
#[ignore] // Long-running
fn power_30k_shift_5pct() {
    ...
    println!("\ntiming-oracle detects 5% shifts at {:.1}% rate", to_power * 100.0);
}
...
fn power_30k_tail() {
    ...
    let to_power = results.power("timing-oracle");
    println!("\ntiming-oracle detects tail effects at {:.1}% rate", to_power * 100.0);
}
...
fn power_30k_same_mean() {
    ...
    let to_power = results.power("timing-oracle");
    println!("\ntiming-oracle detects same-mean effects at {:.1}% rate", to_power * 100.0);
}
...
fn power_500k_shift_1pct() {
    ...
    let to_power = results.power("timing-oracle");
    println!("\ntiming-oracle detects 1% shifts with 500k samples at {:.1}% rate", to_power * 100.0);
}
...
fn power_multi_tool_comparison() {
    ...
    println!("| Configuration | Effect | timing-oracle | dudect | rtlf |");
    println!("|---------------|--------|---------------|--------|------|");
    ...
    for (name, effect) in effects {
        ...
        println!(
            ...
            name,
            results.effect_type,
            results.power("timing-oracle") * 100.0,
