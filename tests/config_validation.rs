...
fn min_effect_negative_panics() {
    let _ = TimingOracle::for_attacker(AttackerModel::AdjacentNetwork).min_effect_ns(-1.0);
}

#[test]
fn min_effect_zero_valid() {
    // Zero is valid (treat all effects as significant)
    let oracle = TimingOracle::for_attacker(AttackerModel::AdjacentNetwork).min_effect_ns(0.0);
    assert_eq!(oracle.config().min_effect_of_concern_ns, 0.0);
}

#[test]
fn min_effect_default_valid() {
    let oracle = TimingOracle::for_attacker(AttackerModel::AdjacentNetwork).min_effect_ns(10.0);
    assert_eq!(oracle.config().min_effect_of_concern_ns, 10.0);
}

#[test]
fn min_effect_large_valid() {
    let oracle = TimingOracle::for_attacker(AttackerModel::AdjacentNetwork).min_effect_ns(1000.0);
...
fn min_effect_infinity_valid() {
    ...
    let oracle = TimingOracle::for_attacker(AttackerModel::AdjacentNetwork).min_effect_ns(f64::INFINITY);
