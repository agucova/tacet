...
tacet ships two JavaScript packages with the same builder-based detection API. Pick based on your runtime:

- **`@tacet/js`** — prebuilt native (N-API) bindings. The fastest option, for **Node.js** and **Bun**. Prebuilt for Linux (x86_64, aarch64), macOS (x86_64, aarch64), and Windows (x86_64).
- **`@tacet/wasm`** — a portable WebAssembly build. Runs on **Node.js**, **Bun**, and **Deno**, plus any platform without a prebuilt native binary. Also published on [JSR](https://jsr.io/@tacet/wasm).

<Aside type="note">
Neither package runs in browsers, which lack the high-precision timers these measurements require.
</Aside>
...
<PackageManagers pkg="@tacet/wasm" pkgManagers={['bun', 'npm', 'pnpm', 'yarn']} />
...
The WASM package (`@tacet/wasm`) exposes the same API; its enum values are accessed via `AttackerModelValues` and `OutcomeValues`.
