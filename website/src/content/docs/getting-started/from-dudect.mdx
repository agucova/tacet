...
If you're familiar with [DudeCT](https://github.com/oreparaz/dudect), this guide shows how to translate your existing patterns to timing-oracle.
...
<TabItem label="timing-oracle (Rust)">
```rust
...
<TabItem label="timing-oracle (C)">
```c
#include <timing_oracle.h>

timing_oracle_inputs_t inputs = {
...
timing_oracle_test(oracle, &inputs, measure_fn, NULL, &outcome);

timing_oracle_print(&outcome);  // Formatted output
...
int main() {
    ...
    timing_oracle_test(oracle, &inputs, measure_aes, NULL, &outcome);
    timing_oracle_print(&outcome);
...
### DudeCT t-statistic → timing-oracle probability

| DudeCT t-statistic | Interpretation | timing-oracle equivalent |
|--------------------|----------------|-------------------------|
...
In timing-oracle, more samples means more confidence in the *same* answer. The probability converges toward the true state: either there's an exploitable leak or there isn't.

## Attacker models vs fixed thresholds

DudeCT uses a fixed t = 4.5 threshold. timing-oracle lets you specify your threat model:
...
Both DudeCT and timing-oracle use the two-class pattern (fixed vs random inputs). The key insight is the same: if code is constant-time, both classes take the same time.

However, timing-oracle's documentation emphasizes:
...
<Aside type="tip">
timing-oracle's adaptive loop automatically determines when to stop, so you don't need to manually specify iteration counts or check t-statistics in a loop.
</Aside>
