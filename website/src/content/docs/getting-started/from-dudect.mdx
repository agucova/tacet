...
<Tabs>
<TabItem label="DudeCT (C)" icon="seti:c">
```c
typedef struct {
    uint8_t *data;
    size_t len;
...
<TabItem label="tacet (Rust)" icon="seti:rust">
```rust
...
<TabItem label="tacet (C)" icon="seti:c">
```c
#include <tacet.h>
...
<Tabs>
<TabItem label="dudect-bencher" icon="seti:rust">
```rust
...
<TabItem label="tacet" icon="seti:rust">
```rust
use tacet::{TimingOracle, AttackerModel, helpers::InputPair};
