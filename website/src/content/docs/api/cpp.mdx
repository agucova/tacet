...
Header-only C++ wrapper for timing-oracle, providing RAII semantics, modern C++ idioms, and a fluent builder API.
...
# Headers at:
#   crates/timing-oracle-c/include/timing_oracle.h
#   bindings/cpp/timing_oracle.hpp
...
try {
    auto result = Oracle::forAttacker(ToAttackerModel::AdjacentNetwork)
        .timeBudget(30s)
        .test(collector);
} catch (const timing_oracle::CalibrationError& e) {
    std::cerr << "Calibration failed: " << e.what() << std::endl;
} catch (const timing_oracle::Error& e) {
...
clang++ -std=c++20 -stdlib=libc++ -O3 -o timing_test timing_test.cpp \
    ...
    -I/path/to/timing-oracle/bindings/cpp \
