...
<ApiLanguageSwitcher currentLanguage="cpp" />
...
```bash
# Install script (Linux/macOS) — includes both tacet.h and tacet.hpp
...
# Artifacts:
# - C header: crates/tacet-c/include/tacet.h
# - C++ wrapper: bindings/cpp/tacet.hpp
# - Library: target/release/libtacet_c.a
# - pkg-config: target/release/tacet.pc
```

**Requirements:** C++20 for `std::span` and `std::chrono_literals`.
