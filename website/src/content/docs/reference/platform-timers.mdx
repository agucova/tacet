...
| Platform | Default Timer | Resolution | PMU Timer | Resolution |
|----------|---------------|------------|-----------|------------|
...
## Why register-based timers?

tacet defaults to register-based timers (`rdtsc`, `cntvct_el0`) because **attackers measure wall-clock time**. When a remote attacker times your API, they observe wall-clock duration—not CPU cycles. `rdtsc` (invariant TSC) and `cntvct_el0` directly measure wall-clock time, matching what attackers can observe.

PMU-based timers (`kperf`, `perf_event`) measure CPU cycles, which can differ from wall-clock time due to frequency scaling. They're available via `TimerSpec::Kperf` or `TimerSpec::PerfEvent` for microarchitectural research or when you need true cycle counts.
