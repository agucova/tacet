...
Use `TimerSpec::RequireCycleAccurate` to request cycle-accurate measurements. This uses kperf on macOS ARM64 and perf\_event on Linux:
...
Operation too fast to measure
  ...
  Recommendation: Use TimerSpec::RequireCycleAccurate with sudo (~1ns resolution)
```

Options:
1. Use `TimerSpec::RequireCycleAccurate` for higher resolution
