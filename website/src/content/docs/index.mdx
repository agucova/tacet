...
Install the native package for Node.js and Bun:

```bash
bun add @tacet/js
```

Or the portable WASM build, which also runs on Deno:

```bash
bun add @tacet/wasm
```

Both expose the same API. See [installation](/getting-started/installation) for more platform details.
</TabItem>
